...
$(document).ready(function() {
	...
	$(".middle-pop-up-btn-box").children("div").click(function(){
		var index = $(this).index();
		var dong = $(this).parents().siblings(".pop-up").eq(index).find(".animation-div-control").length;
		if(dong == 1){
			var canvasId = $(this).parents().siblings(".pop-up").eq(index).find("canvas").get(0).id;
			var animaIndex = animaCreateJs.prototype.getIndexById(canvasId);
			var animaObj = animaCreateJs.prototype.animationArr[animaIndex];
			var anima = animaObj["anima"];
			if(anima.currentFrame == anima.totalFrames) {
			...
			$(this).parents().siblings(".pop-up").eq(index).find(".animation-div-control").hide();
			anima.addEventListener("tick", animaPlaying);
		}
	});
	 ...
	 * 动画播放器 带外框 上下步 带全屏
	 */
	/*$(".animaController canvas").onmouseenter(function(){
		//.log( "enter" );
//		$(this).parent().children(".contrlBar");
	...
	//canvas  无效果
	$(".animaController canvas").mouseout(function() {
		//.log( "mouseout" );
		//		$(this).parent().children(".contrlBar");
	...
	$(".animaController contrlBar").mouseout(function(event) {
		...
		event.preventDefault();
		event.stopPropagation();
		//		$(this).parent().children(".contrlBar").hide(200);
	});
