	...
	return function ( canvasIds ) {
		...
		this.append = function( animaObj ){
			...
			if(loadingBoo) {}
				...
				loadingBoo = true;
				loadAnimation ( animaObj["canvasId"] , animaObj["name"] ,animaObj["lib"] ,animaObj["loadJson"]);
			}
//			
		};
		//私有方法
		...
		function handleCompleteLoop(evt) {
			...
			if(loadIndex<canvasAnimas.length) {
				obj = canvasAnimas[loadIndex];
				check(obj);
				loadAnimation ( obj["canvasId"] , obj["name"] ,obj["lib"] ,obj["loadJson"]);
			}
...
//		loader.addEventListener("progress", handleProgress);
		if( canvasAnimas.length!=0 )
		{
			...
			check(obj);
			loadingBoo = true;
            loader.loadManifest( obj["lib"].properties.manifest);
            console.log( "加载" );
            loadAnimation ( obj["canvasId"] , obj["name"] ,obj["lib"],obj["loadJson"] );
		}
